...
function generateCell(gx, gy) {
  ...
  // 방 타입 분류
  // A(0.00~0.05): 빈 방           B(0.05~0.14): 열주(소형 기둥 2열)
  // C(0.14~0.23): 격자 큰 기둥    D(0.23~0.32): 파티션 방
  // E(0.32~0.40): 밀집 소형 기둥  F(0.40~0.46): 대형 개방
  // G(0.46~0.56): 아케이드        H(0.56~0.65): 산발 중형 기둥
  // I(0.65~0.72): 얇은 슬랩 미로  J(0.72~0.76): 혼합 갤러리
  // K(0.76~0.81): ㄴ자형          L(0.81~0.86): ㄱ자형
  // M(0.86~0.91): ㄷ자형          N(0.91~0.95): 십자형
  // O(0.95~1.00): 혼합 신형 구조물

  const isLargeOpen = zt >= 0.40 && zt < 0.46;
  ...
  // ── E: 밀집 소형 기둥 방 (8%) — 좁은 틈새 미로 ───────────────
  if (zt < 0.40) {
    if (lx % 2 === 1 && ly % 2 === 1 && rand(gx, gy, 42) < 0.68) return 4;
    return 0;
  }

  // ── F: 대형 개방 방 (6%) — 경계벽 합쳐짐, 구조물 없음 ────────
  if (zt < 0.46) return 0;
  ...
  // ── H: 산발 중형 기둥 방 (9%) — 랜덤 중형 기둥 ──────────────
  if (zt < 0.65) {
    if (lx >= 1 && lx <= 8 && ly >= 1 && ly <= 8 && rand(gx, gy, 55) < 0.22) return 8;
    return 0;
  }

  // ── I: 얇은 슬랩 미로 방 (7%) — 가로 슬랩 + 지그재그 통로 ─────
  if (zt < 0.72) {
    if (ly % 4 === 2 && lx >= 1 && lx <= 8) {
      const slabRow = ly >> 2;
  ...
  // ── K: ㄴ자형 방 (5%) — 산발 ㄴ형 벽 ───────────────────────
  if (zt < 0.81) {
    if (lx >= 1 && lx <= 8 && ly >= 1 && ly <= 8 && rand(gx, gy, 71) < 0.20) return 13;
    return 0;
  }

  // ── L: ㄱ자형 방 (5%) — 산발 ㄱ형 벽 ───────────────────────
  if (zt < 0.86) {
    if (lx >= 1 && lx <= 8 && ly >= 1 && ly <= 8 && rand(gx, gy, 72) < 0.20) return 14;
    return 0;
  }

  // ── M: ㄷ자형 방 (5%) — 격자 배치 ㄷ형 벽 ──────────────────
  if (zt < 0.91) {
    if (lx % 5 === 1 && ly % 4 === 1 && lx <= 8 && ly <= 8) return 15;
    return 0;
  }

  // ── N: 십자형 방 (4%) — 격자 배치 십자 기둥 ─────────────────
  if (zt < 0.95) {
    if (lx % 4 === 2 && ly % 4 === 2) return 16;
    return 0;
  }

  // ── O: 혼합 신형 방 (5%) — ㄴ/ㄱ/ㄷ/십자 랜덤 산발 ──────────
